...
def read_flags():
    ...
    parser.add_argument(
        "--epochs", default=5, type=int, help="Number of epochs")

    parser.add_argument("--batch-size", default=4, type=int, help="Batch size")

    parser.add_argument(
        "--logdir", default="logdir_pool", help="Tensorboard log directory")

    parser.add_argument(
        "--reg", type=float, default=0.1, help="L2 Regularizer Term")

    parser.add_argument(
        "--ckdir", default="models_pool", help="Checkpoint directory")
...
def main(flags):
    train = pd.read_csv("./pool_train.csv")
#    train = pd.read_csv("./car_train.csv")

    n_train = train.shape[0]

    test = pd.read_csv("./pool_test.csv")
#    test = pd.read_csv("./car_test.csv")
    ...
    y = tf.placeholder(tf.float32, shape=[None, 224, 224, 1], name="y")
    ...
    train_csv = tf.train.string_input_producer(['pool_train.csv'])
    test_csv = tf.train.string_input_producer(['pool_train.csv'])
    
#    train_csv = tf.train.string_input_producer(['car_train.csv'])
#    test_csv = tf.train.string_input_producer(['car_train.csv'])
